...
ax1.set_ylabel('数据加载时间占比(load/load+train)', fontsize=FONT_SIZE_LABEL, color=COLOR_YLABEL)
...
plt.tight_layout()
# plt.show()
plt.savefig("1.2.1.png", dpi=300, bbox_inches='tight')  # 保存图表为PNG文件

# ====== 柱状图：比例相减 Baseline占比 - Ours占比 ======
ratios = baseline_ratios - ours_ratios
...
bars = ax2.bar(log_batch_sizes, ratios, width=bar_widths, color=COLOR_BAR, alpha=0.85, align='center', label='占比差值')

ax2.set_xlabel('Batch Size', fontsize=FONT_SIZE_LABEL, color=COLOR_Y2LABEL)
ax2.set_ylabel('占比差值 (Baseline - Ours)', fontsize=FONT_SIZE_LABEL, color=COLOR_Y2LABEL)
...
plt.tight_layout()
# plt.show()
plt.savefig("1.2.2.png", dpi=300, bbox_inches='tight')  # 保存图表为PNG文件

...
def plot_individual_node_contribution(df_raw, unit='samples'):
    ...
    for model in MODEL_ORDER:
        ...
        for bs in all_batchsizes:
            ...
            ax.set_xticklabels([f'{n} Nodes' for n in num_nodes_configs], fontsize=FONT_SIZE_TICK, rotation=45, ha='right')
            ax.set_title(f'{model} - Batchsize={bs}', fontsize=FONT_SIZE_LABEL)
            ax.set_xlabel('Nodes Configuration', fontsize=FONT_SIZE_LABEL)
            ax.set_ylabel(f'Individual Node Throughput ({unit_str})', fontsize=FONT_SIZE_LABEL)
            ax.tick_params(axis='y', labelsize=FONT_SIZE_TICK)
            ...
            ax.legend(handles, labels, title='Node Name', bbox_to_anchor=(1.02, 0.5), loc='center left', fontsize=FONT_SIZE_LEGEND)

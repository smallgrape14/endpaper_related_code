...
for i, batch_size in enumerate(batch_sizes):
    ...
    axes[i].set_xlabel('Worker Number', fontsize=12)
    axes[i].set_ylabel('Prefetch Factor', fontsize=12)
...
for batch_size in batch_sizes:
    ...
    plt.xlabel('工作进程数量(Worker Number)', fontsize=12)
    plt.ylabel('预取因子（Prefetch Factor）', fontsize=12)
    plt.tight_layout()

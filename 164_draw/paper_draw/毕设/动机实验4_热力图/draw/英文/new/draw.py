...
fig, axes = plt.subplots(1, len(batch_sizes), figsize=(5,12))
...
for i, batch_size in enumerate(batch_sizes):
    ...
    pivot_table = df_batch.pivot(
        index='worker_num',  # 原为prefetch_factor，现在改为worker_num
        columns='prefetch_factor',  # 原为worker_num，现在改为prefetch_factor
        values='data_per'
    )
    
    # 对行和列进行排序
    pivot_table = pivot_table.sort_index(ascending=True)  # worker_num从小到大
    pivot_table = pivot_table.reindex(sorted(pivot_table.columns), axis=1)  # prefetch_factor从小到大
    ...
    im = sns.heatmap(
        ...
        cbar_kws={'label': 'Data Loading Time Proportion' if i == len(batch_sizes)-1 else ''},
        vmin=vmin,  # 使用统一的最小值
        vmax=vmax,  # 使用统一的最大值
        linewidths=0.5, 
        linecolor='gray',
    ...
    axes[i].set_xlabel('Prefetch Factor', fontsize=12)  # 修改横坐标标签
    axes[i].set_ylabel('Worker Number', fontsize=12)    # 修改纵坐标标签
    ...
    if i == len(batch_sizes)-1:
        ...
        cbar.set_label('Data Loading Time Proportion', size=12)
...
for batch_size in batch_sizes:
    # 筛选当前batch_size的数据
    df_batch = df[df['batch_size'] == batch_size]
    # 在创建透视表前，先筛选出 worker_num 为偶数的行
    df_batch = df_batch[df_batch['prefetch_factor'] % 2 == 0 ]  # 只保留偶数
    
    # 创建透视表 - 横纵坐标互换
    pivot_table = df_batch.pivot(
        index='worker_num',  # 横纵坐标互换
        columns='prefetch_factor',
        values='data_per'
    )
    
    # 对行和列进行排序
    pivot_table = pivot_table.sort_index(ascending=False)  # worker_num从小到大
    pivot_table = pivot_table.reindex(sorted(pivot_table.columns), axis=1)  # prefetch_factor从小到大
    ...
    # 获取当前batch_size的数据范围
    vmin_local = df_batch['data_per'].min()
    vmax_local = df_batch['data_per'].max()
    ...
    im = sns.heatmap(
        ...
        fmt='.3f', 
        cmap='YlOrRd', 
        cbar_kws={'label': 'Data Loading Time Proportion'},
        annot_kws={"size": 24},  # 控制注释字体大小
        linewidths=0.5, 
        linecolor='gray',
        square=False,
        vmin=vmin_local,  # 使用局部范围
        vmax=vmax_local
    )
    ...
    cbar.set_label('Data Loading Time Proportion', size=24)
    
    # plt.title(f'batch_size = {batch_size}', fontsize=20, fontweight='bold')
    plt.xlabel('Prefetch Factor', fontsize=32)  # 横坐标标签
    plt.ylabel('Worker Number', fontsize=32)    # 纵坐标标签

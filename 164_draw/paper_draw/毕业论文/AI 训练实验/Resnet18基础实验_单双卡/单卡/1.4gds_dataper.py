...
COLOR_OURS_LABEL = '#ff7f0e'     # Ours标注字体
...
ours_ratios = ours_data_time / (ours_data_time + ours_train_time)*100

# GDS只有前5组
gds_data_time      = np.array(df['GDSDataTime(s)'])
gds_train_time     = np.array(df['GDSTrainTime(s)'])
gds_ratios = gds_data_time / (gds_data_time + gds_train_time)*100
...
line1, = ax1.plot(batch_sizes, ours_ratios, '-s', color=COLOR_OURS_LINE,
                  linewidth=2, markersize=10, label='OURS')
line2, = ax1.plot(batch_sizes, gds_ratios, '--o', color=COLOR_GDS_LINE,
...
# # gds标注
# for i, (x, y) in enumerate(zip(batch_sizes, gds_ratios)):
#     ax1.annotate(f'{y*100:.2f}%', (x, y),
...
plt.tight_layout()
# plt.show()
plt.savefig("gds_data_per.png", dpi=300, bbox_inches='tight')  # 保存图表为PNG文件

# ====== 柱状图：Ours与GDS占比差值 ======
reduce_percent = gds_ratios - ours_ratios

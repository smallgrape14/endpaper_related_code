...
def plot_performance_metrics(baseline_df, ours_df):
    ...
    for i, metric in enumerate(METRIC_ORDER):
        for j, model in enumerate(models):
            ...
            if j == 0: # 第一列子图设置纵坐标标签
                ax.set_ylabel(f'{METRIC_LABELS_PERCENT[metric]}', fontsize=FONT_SIZE_LABEL, fontproperties='SimHei')

            if i == num_metrics - 1: # 最后一行子图设置横坐标标签
                ax.set_xlabel('批大小（Batch Size）', fontsize=FONT_SIZE_LABEL, fontproperties='SimHei')
            
            ax.set_xticks(x_pos)
            ax.set_xticklabels(all_batchsizes, fontsize=FONT_SIZE_TICK, fontproperties='SimHei')
...
def plot_metric_improvement_absolute(baseline_df, ours_df):
    ...
    for i, metric in enumerate(METRIC_ORDER):
        for j, model in enumerate(models):
            ...
            if j == 0:
                ax.set_ylabel(f'{METRIC_LABELS_IMPROVEMENT[metric]}', fontsize=FONT_SIZE_LABEL, fontproperties='SimHei')
...
def plot_combined_per_metric(baseline_df, ours_df):
    ...
    for metric in METRIC_ORDER:
        ...
        for j, model in enumerate(models):
            ...
            if j == 0:
                ax_top.set_ylabel(f'{METRIC_LABELS_PERCENT[metric]}', fontsize=FONT_SIZE_LABEL, fontproperties='SimHei')
            ax_top.set_title(f'{model}', fontsize=FONT_SIZE_LABEL)
            ax_top.set_xticks(x_pos)
            ax_top.set_xticklabels(all_batchsizes, fontsize=FONT_SIZE_TICK, fontproperties='SimHei')
            ...
            if j == 0:
                ax_bottom.set_ylabel(f'{METRIC_LABELS_IMPROVEMENT[metric]}', fontsize=FONT_SIZE_LABEL, fontproperties='SimHei')
            ax_bottom.set_xlabel('批大小（Batch Size）', fontsize=FONT_SIZE_LABEL, fontproperties='SimHei')
            ax_bottom.set_xticks(x_pos)
            ax_bottom.set_xticklabels(all_batchsizes, fontsize=FONT_SIZE_TICK, fontproperties='SimHei')

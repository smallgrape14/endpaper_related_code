...
# 设置全局字体
plt.rcParams.update({
    # 'font.sans-serif': ['SimSun', 'Arial'],  # 中文字体在前
...
def save_individual_throughput_plots(baseline_df, ours_df, output_path="legend.png"):
    """
    只生成吞吐量图例（不包含曲线）
    """
    ...
    # baseline legend handles
    for thread in baseline_threads:
        line, = ax_dummy.plot([], [],
                              marker='o',
                              linestyle='--',
                              color=baseline_colors[thread],
                              label=f'Baseline (Worker={thread})')
        lines.append(line)
        labels.append(line.get_label())

    # ours legend handle
    ours_line, = ax_dummy.plot([], [],
                               marker='s',
                               linestyle='-',
                               linewidth=2.5,
                               color=ours_color,
                               label='OURS')
    lines.append(ours_line)
    labels.append(ours_line.get_label())

    plt.close(fig_dummy)  # ❗不显示空图

    # ===== 生成 legend 专用 figure =====
    fig_legend = plt.figure(figsize=(10, 1.8))
    ...
    legend = ax_legend.legend(
        ...
        ncol=3,
        fontsize=32,
        prop={'family': 'SimSun', 'size': 26, 'weight': 'bold'},  # 🔥 放大字体
        markerscale=3,     # 🔥 放大 marker
        handlelength=1.8,  # 🔥 拉长线段
    ...
    print(f"legend 已保存: {output_path}")
...
#         ax.set_xlabel('批大小(Batch Size)', fontsize=FONT_SIZE_LABEL,fontproperties='SimSun')
#         ax.set_ylabel('吞吐量 (样本数/秒)', fontsize=FONT_SIZE_LABEL,fontproperties='SimSun')
#         ax.set_xticks(x_pos)
#         ax.set_xticklabels(all_batchsizes, fontsize=FONT_SIZE_TICK,fontproperties='SimSun')

#         if model == 'mobilenet_v2':
#             y_ticks = np.arange(0, 2001, 500)  # 0, 50, 100, ..., 450
#             ax.set_yticks(y_ticks)
#             ax.set_yticklabels(y_ticks, fontsize=FONT_SIZE_TICK,fontproperties='SimSun')
#         elif model == 'shufflenet_v2_x1_0':
#             y_ticks = np.arange(0, 3001, 1000)  # 0, 50, 100, ..., 450
#             ax.set_yticks(y_ticks)
#             ax.set_yticklabels(y_ticks, fontsize=FONT_SIZE_TICK,fontproperties='SimSun')            
#         elif model == 'resnet18':
#             y_ticks = np.arange(0, 3001, 1000)  # 0, 50, 100, ..., 450
...
#             ax.set_yticklabels(y_ticks, fontsize=FONT_SIZE_TICK,fontproperties='SimSun')  
#         elif model == 'resnet50':
#             y_ticks = np.arange(0, 1001, 250)  # 0, 50, 100, ..., 450
#             ax.set_yticks(y_ticks)
#             ax.set_yticklabels(y_ticks, fontsize=FONT_SIZE_TICK,fontproperties='SimSun')  
#         ax.tick_params(axis='y', labelsize=FONT_SIZE_TICK)
#         # ax.legend(loc='best', fontsize=FONT_SIZE_LEGEND, frameon=False)
#         # ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.38), 
#         #           ncol=2, fontsize=FONT_SIZE_LEGEND, frameon=False, 
...
def save_individual_improvement_plots(baseline_df, ours_df, output_dir="individual_plots"):
    ...
    for model in models:
        ...
        ax.set_xlabel('批大小(Batch Size)', fontsize=FONT_SIZE_LABEL, fontproperties='SimSun')
        ax.set_ylabel('吞吐量提升百分比(%)', fontsize=FONT_SIZE_LABEL, fontproperties='SimSun')
        ax.set_xticks(x_pos)
        ax.set_xticklabels(all_batchsizes, fontsize=FONT_SIZE_TICK, fontproperties='SimSun')
        ax.tick_params(axis='y', labelsize=FONT_SIZE_TICK)

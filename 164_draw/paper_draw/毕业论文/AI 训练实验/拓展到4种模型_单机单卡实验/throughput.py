...
def plot_throughput(baseline_df, ours_df):
    ...
    for i, model in enumerate(models):
        ...
        # 使用可调参数设置字体和样式
        ax.set_title(f'{model} - Throughput', fontsize=FONT_SIZE_LABEL)
        ax.set_xlabel('Batch Size', fontsize=FONT_SIZE_LABEL)
        ax.set_ylabel('Throughput (samples/s)', fontsize=FONT_SIZE_LABEL)
        ax.set_xticks(x_pos)
...
def plot_throughput_improve_percent(baseline_df, ours_df):
    ...
    for i, model in enumerate(models):
        ...
        ax.set_title(f'{model} - Throughput Improvement', fontsize=FONT_SIZE_LABEL)
        ax.set_xlabel('Batch Size', fontsize=FONT_SIZE_LABEL)
        ax.set_ylabel('Throughput Improvement (%)', fontsize=FONT_SIZE_LABEL)
        ax.set_xticks(x_pos)
        ax.set_xticklabels(all_batchsizes, fontsize=FONT_SIZE_TICK)
        ax.tick_params(axis='y', labelsize=FONT_SIZE_TICK)

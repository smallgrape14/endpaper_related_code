...
plt.ylabel('数据加载时间占比 (%)', fontsize=FONT_SIZE_LABEL)
plt.xticks(range(len(workers)), [str(w) for w in workers])
y_ticks = np.arange(0, 81, 10)  # 0, 50, 100, ..., 450
print(y_ticks)
plt.yticks(y_ticks, fontsize=FONT_SIZE_TICK)
...
plt.savefig('dual_gpu_data_loading_time.pdf', dpi=300, bbox_inches='tight')
plt.show()
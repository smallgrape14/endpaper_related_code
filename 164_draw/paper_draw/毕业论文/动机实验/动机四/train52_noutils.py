        ...
        if epoch == num_epochs-1 :# 只在最后一个epoch的时候，统计这些
            ...
            with open(avg_csv_file_path, mode='a', newline='') as csv_file:
                ...
                writer.writerow({
                    ...
                    'data_per': np.mean(arr_data)/(np.mean(arr_data)+np.mean(arr_train)),
...
if __name__ == "__main__":
    ...
    if Conn_Mode==0:
        ...
        csv_file_path = "output/epoch_time_BeeGFS.csv"
    elif Conn_Mode==2:
    ...
    ## 每个batch平均值的记录文件
        # 在训练开始之前，初始化 CSV 文件并写入表头
    avg_csv_file_path = "output/avg_batch_time_BeeGFS.csv"
    # 检查文件是否存在并且是否有表头
    ...
    with open(avg_csv_file_path, mode='a', newline='') as csv_file:
        ...
        fieldnames_batch = ['batch_size','worker_num','prefetch_factor',
            'AVG_data','AVG_train','data_per','MIN_data','MAX_data', 'Median_data','p99_data',
            'MIN_train','MAX_train','Median_train','p99_train',

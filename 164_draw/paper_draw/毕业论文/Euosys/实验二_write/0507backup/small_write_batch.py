...
def plot_io_performance_comparison(csv_file_path, io_size_kb=None, output_filename=None):
    ...
    line_our, = ax.plot(thread_all, our_iops_res, 
             ...
             linewidth=linewidths['OUR'], markersize=7, label='OURS(GPU)', 
    ...
    if not df_gds.empty:
        ...
        line_gds, = ax.plot(threads_gds, df_gds['IOPS_avg']/1000, 
                 ...
                 linewidth=linewidths['GDS'], markersize=7, label='GDS', 
    ...
    line_posix, = ax.plot(thread_all, posix_iops_res, 
             ...
             linewidth=linewidths['POSIX'], markersize=7, label='POSIX(CPU)', 
             linestyle=linestyles['POSIX'], zorder=1)
    
    # 设置图表属性
    ax.set_xlabel('ThreadNum', fontsize=FONT_SIZE_LABEL)
    ax.set_ylabel('KIOPS', fontsize=FONT_SIZE_LABEL)  # 将IOPS单位转换为千

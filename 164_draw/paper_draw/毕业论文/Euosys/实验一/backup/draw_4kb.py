...
plt.rcParams['font.serif'] = ['Times New Roman','SimSun' ]  # Times + 宋体
...
# 绘制OUR折线图
threads_our = df_our['Thread_Num']
plt.plot(threads_our, df_our['IOPS_avg']/1000, 
         marker=markers['OUR'], color=colors['OUR'], 
         linewidth=2.2, markersize=8, label='OURS(GPU)', 
         linestyle=linestyles['OUR'])

# 设置图表属性
plt.xlabel('ThreadNum', fontsize=FONT_SIZE_LABEL)
plt.ylabel('KIOPS', fontsize=FONT_SIZE_LABEL)  # 将IOPS单位转换为千
...
# 保存图表
plt.savefig('Read_iops_4kb_comparison.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.savefig('Read_iops_4kb_comparison.pdf', dpi=300, bbox_inches='tight', facecolor='white')

...
# 添加标签和标题
ax.set_xlabel('Batch Size', fontdict = font)
ax.set_ylabel('Time (minutes)', fontdict = font)
# ax.set_title('Data Processing and Training Time Comparison', fontsize=16, pad=20)
ax.set_xticks(x)
ax.set_xticklabels(batch_size,fontdict = font)
# ax.legend(loc='upper right', bbox_to_anchor=(1, 1),fontsize=13)
ylist=[]
for i in range(9):
    ylist.append(10*i)
ax.set_yticks(ylist)
ax.set_yticklabels(ylist,fontdict = font)
...
# 保存和显示图表
plt.savefig('figure/batch_size_comparison_now_csv.png', dpi=300, bbox_inches='tight')
# plt.show()
plt.savefig('figure/batch_size_comparison_now_csv.pdf', format='pdf', bbox_inches='tight')

...
ax.bar(x - bar_width/2, df['Baseline_data'], bar_width,
       bottom=df['Baseline_train'], color=baseline_data_color, 
       edgecolor='white', 
...
ax.bar(x + bar_width/2, df['Ours_data'], bar_width,
       bottom=df['Ours_train'], color=our_data_color, 
...
ax.set_xticklabels(df['batch_size'],fontsize=14)
...
# 保存和显示图表
plt.savefig('batch_size_comparison_now.png', dpi=300, bbox_inches='tight')
plt.show()

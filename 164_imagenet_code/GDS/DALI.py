...
import matplotlib.pyplot as plt  # 核心导入语句
import torch
...
MAX_BATCH=2000 #定义上限Batch_NUM数量 
...
def image_processing_pipeline(data_dir,crop_size_x,crop_size_y,numpy_files,labels):
    # 读取文件
    images, labels_data = fn.readers.file(
        file_root=data_dir,
        random_shuffle=True,
        name="label_reader"
        # recursive=True        # 搜索子目录
    )
    # images=fn.readers.numpy(device="gpu", file_root=data_dir, files=numpy_files,shuffle_after_epoch=True)
    # images=fn.readers.numpy(device="gpu", file_root=data_dir, files=numpy_files, shuffle_after_epoch=True,name="gpu_reader")

    
    # 添加标签（根据图片示例扩展）
    # labels_data = fn.external_source(
    #     source=lambda source_info: labels,  # 使用预定义的标签列表
    #     # batch=False,  # 整个列表作为单个样本
    #     batch=True,
    #     dtype=types.INT32 ,
    #     name="label_source"
    ...
    # )
    # 只保留标签，忽略图像路径
    # images_data, labels_data = fn.readers.file(
    #     file_root=data_dir,
    #     random_shuffle=True,
    #     name="Reader"
    # )
    return images, labels_data# 标签也需转移至GPU

    # return images.gpu(), labels_data.gpu()# 标签也需转移至GPU
...
# 预生成路径和标签列表（兼容格式）
# precomputed_files = ["class0/img0.npy", "class1/img1.npy"]
# precomputed_labels = [np.array([0], dtype=np.int32), np.array([1], dtype=np.int32)]  # 转换为NumPy数组

# 创建分批数据生成器
# def data_generator():
#     batch_size=16 #【DEBUG_XYP】
#     all_data = list(zip(precomputed_files, precomputed_labels))
#     while True:  # 无限循环以支持多epoch训练
#         random.shuffle(all_data)  # 每个epoch打乱数据
#         batch_files, batch_labels = [], []
#         for file, label in all_data:
#             batch_files.append(file)
#             batch_labels.append(label)
#             if len(batch_files) == batch_size:  # 达到批次大小
#                 yield (batch_files, batch_labels)
#                 batch_files, batch_labels = [], []
#         if batch_files:  # 处理剩余样本
#             yield (batch_files, batch_labels)
    ...
    for cls in classes:
        ...
        for file in os.listdir(cls_dir):
            if file.endswith(".npy"):
                rel_path = os.path.join(cls, file)  # 保留相对路径
                label = np.array([class_to_idx[cls]], dtype=np.int32)
                file_label_pairs.append((rel_path, label))
    return file_label_pairs

# def data_generator(data_dir, batch_size,batch_files,all_data,total_samples):
def data_generator(data_dir, batch_size,all_data,total_samples):

    # all_data = scan_directory(data_dir)
    # total_samples = len(all_data)
    
    while True:
        # 没获取一个batch就打乱 ，影响性能，并且file 和标签对不上
        ...
        for i in range(0, total_samples, batch_size):
            ...
            yield list(batch_files), list(batch_labels)  # 转换为列表
            # print("文件路径类型:", type(batch_files[0]))  # 应为 <class 'str'>
            # print("标签类型:", type(batch_labels[0]))     # 应为 <class 'numpy.ndarray'>

def index_generator(total_samples, batch_size):
    ...
    while True:
        ...
        for i in range(0, total_samples, batch_size):
            ...
            if len(batch_indices) == batch_size:
                yield batch_indices.astype(np.int32)  # 输出整型索引
USE_GDS=1
@pipeline_def(batch_size=16, num_threads=4, device_id=0)
def gds_pipeline(data_dir,files,labels,total_samples,all_data,pip_batchsize):
    
    # batch_files=[]
    # # 使用fn.external_source生成数据节点
    _, labels = fn.external_source(
        # source=lambda: data_generator(data_dir,batch_size=16),
        source=data_generator(data_dir,batch_size=pip_batchsize,all_data=all_data,total_samples=total_samples),
        num_outputs=2,  # 指定输出数量
        # cycle='raise',  # 数据用完后自动重启
        device='cpu'     # CPU处理标签
        # batch=False
    )
    # labels = fn.external_source(
    #     source=index_generator(total_samples,batch_size=16),
    #     num_outputs=1,  # 指定输出数量
    #     # cycle='raise',  # 数据用完后自动重启
    #     device='cpu'     # CPU处理标签
    #     # batch=False
    # )
    ...
    #     device="gpu",  # 标签也可放在GPU
    #     file_root=data_dir,
    #     files=labels,
    #     cache_header_information=False
    # )
    # 关键：使用file reader自动提取标签
    # _, labels = fn.readers.file(
    #     file_root=data_dir,
    #     files=files
    #     # random_shuffle=True,    # 打乱顺序
    #     # with_label=True,        # 自动从目录名生成标签
    #     # file_filter="*.npy"    # 过滤文件格式
    # )
    # labels = ops.ExternalSource(source=lambda: labels)
    if USE_GDS == 1:
...
class DaliGDSDataset(IterableDataset):
    def __init__(self, pipeline, size):
        ...
        self.num_classes=2
        # print(f"batchsize ={self.batch_size} , total_sample={self.total_samples}  ")
    def __len__(self):
        """计算每个epoch的批次数"""
        batch_num=(self.total_samples + self.batch_size - 1) // self.batch_size
    ...
    def __iter__(self):
        ...
        for batch in self.dali_iterator:
            # print("dataset-----------1-----------")
            images = batch[0]["images"]  # shape: [batch, 3, H, W]
            # print("dataset-----------2-----------")
            targets = batch[0]["labels"].squeeze(-1).long()
            # targets = torch.randint(0, self.num_classes, (images.size(0),), device=images.device)  # 模拟标签
            # print("dataset-----------3-----------")
...
#加载数据集合的关键代码-------------------
        ...
        for cls_name in classes:
            ...
            if not os.path.isdir(cls_dir):
                continue
            for fpath in glob.glob(os.path.join(cls_dir, "*.npy")):
                numpy_files.append(os.path.relpath(fpath, train_dir))  # 相对路径
                labels_list.append(class_to_idx[cls_name])
                # if(file_idx % 1300==0):
                #     print(f"{file_idx} th file : {numpy_files[file_idx]} ,label is {labels_list[file_idx]}")
                file_idx+=1
        file_num=file_idx
        # print("----------2------------")
        ...
        # 4. 创建标签数组（避免使用列表）
        # labels_np = np.array([
        #     class_to_idx[os.path.dirname(fpath)]  # 从相对路径中提取父目录名
        #     for fpath in numpy_files
        # ], dtype=np.int32)

        # 构建管道（必须操作）
        # pipe = image_processing_pipeline(train_dir, 244,244,batch_size=args.batch_size,num_threads=args.workers,numpy_files=numpy_files,labels=labels_np)
        # pipe.build()  # 此步骤会暴露Reader元数据
        
        # 解决方案2: 验证Reader是否暴露元数据（图片中隐含的步骤）
        # if "gpu_reader" not in pipe.reader_meta():
        #     print("警告: 可能需要调整Reader命名或返回方式")
        # else:
        #     print("Reader 暴露了元数据")
        
        # train_data = DALIGenericIterator(
        #     [image_processing_pipeline(train_dir, 244,244,batch_size=args.batch_size,num_threads=args.workers,numpy_files=numpy_files,labels=labels)], ['images', 'target'],
        #     reader_name='gpu_reader'
        # )

        p = gds_pipeline(train_dir,
                        batch_size=args.batch_size,
                        num_threads=args.workers,
                        files=numpy_files,
                        labels=labels_list,
                        total_samples=file_num,
                        all_data=scan_directory(train_dir),
                        pip_batchsize=args.batch_size,
                        # device_id=args.gpu)
                        device_id=0)
        ...
        dali_dataset = DaliGDSDataset(p, size=len(numpy_files))
        # print("----------5-----------")

        train_data = DataLoader(dali_dataset, batch_size=None)  # batch 已由 DALI 管理
                    # batch_size=32
                    # num_workers=2, 
                    # pin_memory=True,
                    # prefetch_factor=1
    ...
    # for epoch in range(1,10):
        ...
        #DALI 预处理的加载模式
        train(train_data, model, criterion, optimizer, epoch, device, args,file_num,batch_data_list,batch_train_list,batch_todevice_list,batch_idx)

        # print(f"now batch_idx is {batch_idx}")
    ...
    # epoch 结束开始统计每个batch 的AVG 数据 data 和 train
    csv_file_path = "result/0820/avg_batch_time_BeeGFS_GDS.csv"
        # 检查文件是否存在并且是否有表头
    ...
    with open(csv_file_path, mode='a', newline='') as csv_file:
        fieldnames = ['batch_size','worker_num','batchnum','prefetch_factor','data_time','training_time']#, 'data_todevice_time']
    ...
    with open(csv_file_path, mode='a', newline='') as csv_file:
            ...
            writer.writerow({
                ...
                'prefetch_factor':1,
                'batchnum':MAX_BATCH,
                'data_time': np.mean(arr_data),
                # 'data_todevice_time': np.mean(arr_train),
                'training_time':  np.mean(arr_train)
            })




def train(train_loader, model, criterion, optimizer, epoch, device, args,file_num,batch_data_list,batch_train_list,batch_todevice_list,batch_idx):
    ...
    model.train()
    # epoch 结束开始统计每个batch 的AVG 数据 data 和 train
    batch_csv_file_path = f"result/0820/batch_time_BeeGFS_GDS_B{args.batch_size}_w{args.workers}.csv"
        # 检查文件是否存在并且是否有表头
    write_header = not os.path.exists(batch_csv_file_path)
    if not write_header:
        with open(batch_csv_file_path, mode='r', newline='') as csv_file:
            reader = csv.reader(csv_file)
            try:
                header = next(reader)
                write_header = len(header) == 0 or header[0] != 'batch_size'
    ...
    with open(batch_csv_file_path, mode='a', newline='') as csv_file:
        fieldnames_batch = ['batch_size','worker_num','prefetch_factor','batch_idx','data_time','training_time']#, 'data_todevice_time']
    ...
    for i, (images, target) in enumerate(train_loader): #遍历 train_loader 中的每个批次数据。enumerate 用于获取批次索引 i 和批次数据 (images, target)
        ...
        # print("inputs device:", images.device)  # 输出如 `cpu` 或 `cuda:0`
        # print("labels device:", target.device)
        ...
        batch_end = time.time()
        train_time.update(batch_end-data_end)
        ...
        with open(batch_csv_file_path, mode='a', newline='') as csv_file:
            ...
            writer.writerow({
                ...
                'prefetch_factor':1,
                'batch_idx':batch_idx,
                'data_time': data_end - end,
                # 'data_todevice_time': np.mean(arr_train),
                'training_time':  batch_end-data_end
            })
        if(batch_idx>MAX_BATCH):
            break
        end = time.time()

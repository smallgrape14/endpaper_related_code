...
def gds_pipeline(data_dir,files,labels,pipe_batchsize):
    
    batch_files=[]
    # 使用fn.external_source生成数据节点
    _, labels = fn.external_source(
        # source=lambda: data_generator(data_dir,batch_size=16),
        source=data_generator(data_dir,batch_size=pipe_batchsize,batch_files=batch_files),
    ...
    print("len of batch_files =",len(batch_files))
    print("type(batch_files) = ",type(batch_files))
...
#加载数据集合的关键代码-------------------
        ...
        p = gds_pipeline(train_dir,
                        ...
                        files=numpy_files,
                        labels=labels_list,
                        pipe_batchsize=args.batch_size,
                        device_id=args.gpu)
        print("----------1-----------")
    ...
    # for epoch in range(1,10):
        ...
        #DALI 预处理的加载模式
        train(train_data, model, criterion, optimizer, epoch, device, args,file_num)
...
def train(train_loader, model, criterion, optimizer, epoch, device, args,file_num):

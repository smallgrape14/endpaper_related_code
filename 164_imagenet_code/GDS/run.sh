...
# Train_Dir="/mnt/beegfs/imagenet_10_decode"
...
for CONN_MODE in 1 #1 2 3
do
    for batch_size in 64 #16 32 64 128 256 512  #16 32 #64  128 256 512 
    do
        for worker_num in 16 #16 #32 #16 32 #16 32 1 8 16 32 64 #256  #32 64 #128  #32 40 64 128
        do  
            for prefetch_factor in 1 #2 3 4
            do
                Output_Name="result/0820/output/output_b${batch_size}_w${worker_num}.txt"
                echo $Output_Name
                # 执行 Python 脚本并传递参数
                # sudo python3 $PYTHON_SCRIPT --batch_size $BATCH_SIZE --epochs $EPOCH_NUM --workers $worker_num
                ...
                sudo python $PYTHON_SCRIPT -a resnet18 $Train_Dir -j $worker_num --epochs $EPOCH_NUM -b $batch_size > $Output_Name #--gpu $gpu_id
...
# do
#     for worker_num in 16 #0 8 32 40 #0 4 8 16 32 64 
#     do  

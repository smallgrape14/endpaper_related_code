...
parser.add_argument('--max_batch', default=100, type=int,
                    help='max batch num in one epoch')
best_acc1 = 0
...
def main():
    ...
    if torch.cuda.is_available():
        ngpus_per_node = GPU_NUM #torch.cuda.device_count()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print(f"worker_num : {args.workers}")
    ...
    if gpu == 0: # 只在主进程（GPU 0）写入统计结果
        if Open_DEBUG == 1:
            if Close_Pin == 1:
                avg_csv_file_path = f"dist/Close_avg_batch_time_dist_debug.csv"
            else:
                avg_csv_file_path = f"dist/avg_batch_time_dist_debug.csv"

            # avg_csv_file_path = f"dist/avg_batch_time_dist_debug_b{args.batchsize}_w{args.workers}.csv"

        else: 
            # avg_csv_file_path = f"dist/avg_batch_time_dist_b{args.batch_size}_w{args.workers}.csv"
            if Close_Pin == 1:
                avg_csv_file_path = f"dist/closePinM_avg_batch_time_dist.csv"
            else:
                avg_csv_file_path = f"dist/avg_batch_time_dist.csv"

        write_header = not os.path.exists(avg_csv_file_path)
        with open(avg_csv_file_path, mode='a', newline='') as csv_file:
            fieldnames_batch = ['batch_size','batchnum','worker_num', 'AVG_data', 'MIN_data', 'MAX_data', 
                                'Median_data', 'p99_data', 'AVG_train', 'MIN_train', 'MAX_train', 
                                'Median_train', 'p99_train']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames_batch)
            if write_header:
        ...
        with open(avg_csv_file_path, mode='a', newline='') as csv_file:
            ...
            writer.writerow({
                ...
                # 'batchnum':args.epochs*Max_batch,
                'batchnum':args.epochs*args.max_batch,
...
def train(train_loader, model, criterion, optimizer, epoch, device, args,batch_data_list, batch_train_list):
    ...
    if args.gpu == 0:
        if Open_DEBUG==0:
            if Close_Pin == 0:
                batch_csv_file_path = f"dist/each_batch_time_dist_b{args.batch_size}_workernum{args.workers}.csv"
            else: # 关闭pin_memory
                batch_csv_file_path = f"dist/closePinM_each_batch_time_dist_b{args.batch_size}_workernum{args.workers}.csv"
        else:
            if Close_Pin == 0:
                batch_csv_file_path = f"dist/each_batch_time_dist_b{args.batch_size}_workernum{args.workers}_debug.csv"
            else: # 关闭pin_memory
                batch_csv_file_path = f"dist/closePinM_each_batch_time_dist_b{args.batch_size}_workernum{args.workers}_debug.csv"

        write_header = not os.path.exists(batch_csv_file_path)
        with open(batch_csv_file_path, mode='a', newline='') as csv_file:
            fieldnames_eachbatch = ['batch_size','worker_num', 'batch_idx','batch_time','avg_batch','data_time','avg_data','train_time','avg_train']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames_eachbatch)
            if write_header:
                writer.writeheader()
    # switch to train mode
    model.train()
    batchnum=0
    total_batches=int(len(train_loader)/(args.batch_size))
    if Open_NVTX == 0:
        ...
        for i, (images, target) in enumerate(train_loader): #遍历 train_loader 中的每个批次数据。enumerate 用于获取批次索引 i 和批次数据 (images, target)
            ...
            arr_train=np.array(batch_train_list)
            if args.gpu==0:
                print(f" {i} th batch,total_time: {(batch_end- train_begin+data_end-end):.6f}({(np.mean(arr_data)+np.mean(arr_train)):.6f}),data_time: {(data_end - end):.6f}({(np.mean(arr_data)):.6f}),train_time: {(batch_end- train_begin):.6f}({(np.mean(arr_train)):.6f})")
            ...
            if args.gpu==0:
                with open(batch_csv_file_path, mode='a', newline='') as csv_file:
                    ...
                    writer.writerow({
                        ...
                        'worker_num': args.workers,
                        'batch_idx':batchnum,
                        'batch_time':batch_end- train_begin+data_end-end,
                        'avg_batch':np.mean(arr_data)+np.mean(arr_train),
                        'data_time':data_end - end,
                        'avg_data':np.mean(arr_data),
                        'train_time':batch_end- train_begin,
                        'avg_train':np.mean(arr_train),

                    })
            # if(batchnum>Max_batch):
            if(batchnum>args.max_batch):

                # print("达到最大 batchnum 了")
                break
                # print("@@@Write 成功")
        ...
        with nvtx.range("iterate dataloader"):
            for batch_idx in range(total_batches):
                with nvtx.range("enumerate_batch"):  # NVTX标记枚举范围
                    images, target = next(iter(train_loader))  # 直接获取下一批次
            # for i, (images, target) in enumerate(train_loader): #遍历 train_loader 中的每个批次数据。enumerate 用于获取批次索引 i 和批次数据 (images, target)
                ...
                arr_train=np.array(batch_train_list)
                if args.gpu==0:
                    print(f" {batch_idx} th batch,total_time: {(batch_end- train_begin+data_end-end):.6f}({(np.mean(arr_data)+np.mean(arr_train)):.6f}),data_time: {(data_end - end):.6f}({(np.mean(arr_data)):.6f}),train_time: {(batch_end- train_begin):.6f}({(np.mean(arr_train)):.6f})")

...
import sys
NO_first_batch=0 #表示统计时间（数据加载还是训练）都去掉首个batch（每个epoch都是这样）
BATCH_SIZE=256
BATCH_NUM=64
EPOCH_NUM =1
WORKER_NUM=0
...
def get_gpu_usage(device_id=0):
    ...
    return {
        'load': gpu.load * 100,  # GPU利用率百分比
        'memory_used': gpu.memoryUsed,  # 已使用显存(MB)
        'memory_total': gpu.memoryTotal  # 总显存(MB)
    }

def get_pytorch_gpu_stats(device=0):
    """
    获取PyTorch相关的GPU统计信息
    """
    torch.cuda.synchronize(device)  # 确保所有CUDA操作完成
    return {
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        return res
# 新增：创建GPU平均值计算函数
def calculate_average_metrics(phase_metrics):
    """计算指定阶段GPU指标的平均值"""
    ...
    for batch_metrics in phase_metrics:
        total_load += batch_metrics['gpu_usage']['load']
        total_memory_used += batch_metrics['gpu_usage']['memory_used']
        total_memory_allocated += batch_metrics['torch_stats']['memory_allocated']
        total_memory_cached += batch_metrics['torch_stats']['memory_cached']
        count += 1
    
    return {
        'avg_load': total_load / count,
        'avg_memory_used': total_memory_used / count,
        'avg_memory_allocated': total_memory_allocated / count,
        'avg_memory_cached': total_memory_cached / count,
        'batch_count': count
    }
# def train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs=10):
def train_model(model, criterion, optimizer, train_loader,device,scheduler,print_freq=10, num_epochs=10,prefetch_factor=0,workernum=0,gpu_id=0):
    # 清理页面缓存、目录项和inode
    batch_data_list=[]
    batch_train_list=[]
    batch_todevice_list=[]
    ...
    gpu_metrics = {
        ...
        'training': [],
        'epoch_times': [],
        'average_metrics': []  # 新增：存储每个epoch的平均指标
    }
    if Clean_Cache == 1:
    ...
    for epoch in range(num_epochs):
        ...
        if Open_NVTX==1:
            ...
            for i, (inputs, labels) in enumerate(train_loader):
                ...
                data_time.update(data_end - batch_start) # 对应日志中的"Data"列 即数据加载和预处理的时间
                # 记录数据加载阶段的GPU状态
                gpu_usage_data_loading = get_gpu_usage(gpu_id)
                torch_gpu_stats_data_loading = get_pytorch_gpu_stats(gpu_id)
                epoch_gpu_metrics['data_loading'].append({
                    'gpu_usage': gpu_usage_data_loading,
                    'torch_stats': torch_gpu_stats_data_loading,
                    'duration': data_end - batch_start
                })
                ...
                gpu_usage_training = get_gpu_usage(gpu_id)
                torch_gpu_stats_training = get_pytorch_gpu_stats(gpu_id)
                epoch_gpu_metrics['training'].append({
                    'gpu_usage': gpu_usage_training,
                    'torch_stats': torch_gpu_stats_training,
                ...
                batch_start = time.time()
        # 存储每个epoch的GPU指标
        gpu_metrics['data_loading'].append(epoch_gpu_metrics['data_loading'])
        gpu_metrics['training'].append(epoch_gpu_metrics['training'])
        gpu_metrics['epoch_times'].append(epoch_total_time)
        
        # 新增：在每个epoch结束时计算平均值
        epoch_avg_metrics = {
            'epoch': epoch,
            'data_loading': calculate_average_metrics(epoch_gpu_metrics['data_loading']),
            'training': calculate_average_metrics(epoch_gpu_metrics['training']),
            'epoch_duration': epoch_total_time
        }
        
        # 存储平均值数据
        gpu_metrics['average_metrics'].append(epoch_avg_metrics)
        
        # 新增：打印本epoch的平均GPU指标
        print(f'Epoch {epoch} GPU负载平均值:')
        print(f'  数据加载阶段: {epoch_avg_metrics["data_loading"]["avg_load"]:.2f}%')
        print(f'  训练阶段: {epoch_avg_metrics["training"]["avg_load"]:.2f}%')
        print(f'  数据加载内存使用: {epoch_avg_metrics["data_loading"]["avg_memory_used"]/1024:.2f} MB')
        print(f'  训练内存使用: {epoch_avg_metrics["training"]["avg_memory_used"]/1024:.2f} MB')
        # 将GPU监控数据写入CSV（包括平均值）
        with open(avg_gpu_metrics_file, mode='a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=avg_gpu_metrics_fieldnames)
            # 写入详细批次数据
            ...
            writer.writerow({
                'epoch': epoch,
                'phase': 'average_data_loading',
                'gpu_load': epoch_avg_metrics['data_loading']['avg_load'],
                'memory_used': epoch_avg_metrics['data_loading']['avg_memory_used'],
                'memory_allocated': epoch_avg_metrics['data_loading']['avg_memory_allocated'],
                'memory_cached': epoch_avg_metrics['data_loading']['avg_memory_cached'],
                'duration': epoch_avg_metrics['epoch_duration'],
            ...
            writer.writerow({
                'epoch': epoch,
                'phase': 'average_training',
                'gpu_load': epoch_avg_metrics['training']['avg_load'],
                'memory_used': epoch_avg_metrics['training']['avg_memory_used'],
                'memory_allocated': epoch_avg_metrics['training']['avg_memory_allocated'],
                'memory_cached': epoch_avg_metrics['training']['avg_memory_cached'],
                'duration': epoch_avg_metrics['epoch_duration'],
                'batch_size': BATCH_SIZE,
                'worker_num': WORKER_NUM,
                'prefetch_factor': prefetch_factor
        ...
        # 将GPU监控数据写入CSV
        with open(gpu_metrics_file, mode='a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=gpu_metrics_fieldnames)
            for phase in ['data_loading', 'training']:
                for batch_metrics in epoch_gpu_metrics[phase]:
                    writer.writerow({
                        'epoch': epoch,
                        'phase': phase,
                        'gpu_load': batch_metrics['gpu_usage']['load'],
                        'memory_used': batch_metrics['gpu_usage']['memory_used'],
                        'memory_allocated': batch_metrics['torch_stats']['memory_allocated'],
                        'memory_cached': batch_metrics['torch_stats']['memory_cached'],
                        'duration': batch_metrics['duration'],
                        'batch_size': BATCH_SIZE,
                        'worker_num': WORKER_NUM,
                        'prefetch_factor': prefetch_factor
                    })
...
if __name__ == "__main__":
    ...
    # GPU监控文件初始化
    # gpu_metrics_file = f"gpu_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    gpu_metrics_file = f"gpu_metrics_batch{args.batch_size}_worker{WORKER_NUM}.csv"

    gpu_metrics_fieldnames = [
        'epoch', 'phase', 'gpu_load', 'memory_used', 'memory_allocated', 
        'memory_cached', 'duration', 'batch_size', 'worker_num', 'prefetch_factor'
    ]
    
    with open(gpu_metrics_file, mode='w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=gpu_metrics_fieldnames)
        writer.writeheader()
    
    avg_gpu_metrics_file = f"avg_gpu_metrics.csv"

    avg_gpu_metrics_fieldnames = [
        'epoch', 'phase', 'gpu_load', 'memory_used', 'memory_allocated', 
        'memory_cached', 'duration', 'batch_size', 'worker_num', 'prefetch_factor'
    ]
    
    with open(avg_gpu_metrics_file, mode='w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=avg_gpu_metrics_fieldnames)
        writer.writeheader()
    ...
    if Conn_Mode==0:
        ...
        csv_file_path = "output/0819/epoch_time_BeeGFS.csv"
    elif Conn_Mode==2:
    ...
    ## 每个batch平均值的记录文件
        # 在训练开始之前，初始化 CSV 文件并写入表头
    avg_csv_file_path = "output/0819/avg_batch_time_BeeGFS.csv"
    # 检查文件是否存在并且是否有表头
    ...
    if Conn_Mode==0:
        ...
        # output_dir = "/mnt/beegfs/test_dir" #两个类别的数据集 替换为你的输出目录 beegfs 挂载目录
        # output_dir = "/mnt/beegfs/imagenet_10_decode" # 替换为你的输出目录 beegfs 挂载目录
        output_dir = "/mnt/beegfs/ImageNet_all" #1000个类别的数据集
    ...
    print("开始训练")
    if MODE_Train == 3:
        train_model(model, criterion, optimizer, train_loader,device,scheduler,print_freq, num_epochs=EPOCH_NUM,prefetch_factor=prefetch,workernum=WORKER_NUM,gpu_id=gpu_id)
    # elif MODE_Train == 2:

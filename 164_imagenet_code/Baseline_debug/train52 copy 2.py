...
def train_model(model, criterion, optimizer, train_loader,device,scheduler,print_freq=10, num_epochs=10,prefetch_factor=0,workernum=0):
    # 清理页面缓存、目录项和inode
    batch_data_list=[]
    batch_train_list=[]
    batch_todevice_list=[]
    ...
    for epoch in range(num_epochs):
        ...
        if Open_NVTX==1:
            ...
            for i, (inputs, labels) in enumerate(train_loader):
                ...
                data_time.update(data_end - batch_start) # 对应日志中的"Data"列 即数据加载和预处理的时间
                # 记录数据加载阶段的GPU状态
                gpu_usage_data_loading = get_gpu_usage()
                torch_gpu_stats_data_loading = get_pytorch_gpu_stats()
                ...
                gpu_usage_training = get_gpu_usage()
                torch_gpu_stats_training = get_pytorch_gpu_stats()
...
if __name__ == "__main__":
    ...
    print("开始训练")
    if MODE_Train == 3:
        train_model(model, criterion, optimizer, train_loader,device,scheduler,print_freq, num_epochs=EPOCH_NUM,prefetch_factor=prefetch,workernum=WORKER_NUM)
    # elif MODE_Train == 2:

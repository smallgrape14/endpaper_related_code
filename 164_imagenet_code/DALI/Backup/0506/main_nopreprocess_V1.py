...
# 定义 DALI 预处理管道
@pipeline_def
def train_image_processing_pipeline():
    images, labels = fn.external_source(source=train_dali_external_source, num_outputs=2)
    images = fn.cast(images, dtype=types.UINT8)
    images = fn.reshape(images, shape=(3, 256, 256))  # 假设原始图片大小为 256x256

    # 将数据移动到 GPU
    images = fn.copy(images, device="gpu")
    # 解码图片
    images = fn.image_decoder(images, device="gpu", output_type=types.RGB)
    ...
    # 随机水平翻转
    # images = fn.random.hflip(images, probability=0.5, device="gpu")
    # 随机水平翻转
    ...
    # 标准化
        # 标准化
    mean = types.Constant([0.485 * 255, 0.456 * 255, 0.406 * 255], dtype=types.FLOAT)
    stddev = types.Constant([0.229 * 255, 0.224 * 255, 0.225 * 255], dtype=types.FLOAT)
    images = fn.normalize(
        images,
        mean=mean,
        stddev=stddev,
...
@pipeline_def
def val_image_processing_pipeline():
    images, labels = fn.external_source(source=val_dali_external_source, num_outputs=2)
    images = fn.cast(images, dtype=types.UINT8)
    images = fn.reshape(images, shape=(3, 256, 256))  # 假设原始图片大小为 256x256
...
# 定义 DALI external source
def train_dali_external_source():
    for images, labels in train_loader:
        return images,labels
        yield images.to('cpu'), labels.to('cpu')
        yield images.to('gpu'), labels.to('gpu')


def val_dali_external_source():
...
#加载数据集合的关键代码-------------------
        ...
        train_dataset = CustomDataset(root_dir=traindir)
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers)

        # 创建 DALI 管道
        global dali_train_loader
        dali_pipeline = train_image_processing_pipeline(batch_size=args.batch_size, num_threads=args.workers, device_id=0)
        dali_pipeline.build()
        dali_train_loader = DALIGenericIterator(pipelines=[dali_pipeline], output_map=["images", "labels"], size=len(train_dataset))

        # 创建自定义数据集并加载到 CPU 内存中
        val_dataset = CustomDataset(root_dir=valdir)
        val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers)

        # 创建 DALI 管道
        global dali_val_loader
        dali_pipeline_val = val_image_processing_pipeline(batch_size=args.batch_size, num_threads=args.workers, device_id=0)
        dali_pipeline_val.build()
        dali_val_loader = DALIGenericIterator(pipelines=[dali_pipeline_val], output_map=["images", "labels"], size=len(val_dataset))
    # 如果启用了评估模式（args.evaluate），直接调用 validate 函数对模型进行评估，并退出函数
    if args.evaluate:
        # validate(val_loader, model, criterion, args)
        validate(dali_val_loader, model, criterion, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #DALI 预处理的加载模式
        train(dali_train_loader, model, criterion, optimizer, epoch, device, args)
        ...
        acc1 = validate(dali_val_loader, model, criterion, args)

#decode 在CPU/GPU上解码，数据还需要转移到GPU上
import argparse
import os
import random
import shutil
...
# 定义 DALI 预处理管道
@pipeline_def(batch_size=16, num_threads=1, device_id=0)
def image_processing_pipeline(data_dir,crop_size_x,crop_size_y):
    ...
    images = fn.resize(images, resize_x=crop_size_x, resize_y=crop_size_y)
...
#加载数据集合的关键代码-------------------
        ...
        train_data = DALIGenericIterator(
            [image_processing_pipeline(train_dir, 244,244)], ['images', 'target'],
            reader_name='Reader'
        )
        val_data = DALIGenericIterator(
            [image_processing_pipeline(val_dir, 244,244)], ['images', 'target'],
            reader_name='Reader'
        )

...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dummy: #@@ 如果启用了虚拟数据（args.dummy），使用 datasets.FakeData 生成虚拟训练和验证数据集。
    ...
    else: #@@ 从指定路径加载训练和验证数据集，并应用数据增强和标准化。
        traindir = os.path.join(args.data, 'train')
        valdir = os.path.join(args.data, 'val')
        
        #@@ 使用ImageNet标准化的均值和标准差
        ...
        train_dataset = datasets.ImageFolder(
            traindir,
            transforms.Compose([
                transforms.RandomResizedCrop(224), # 随机裁剪缩放:随机区域裁剪并缩放到224x224，增强尺度不变性
                transforms.RandomHorizontalFlip(), # 随机水平翻转:50%概率镜像翻转，增加数据多样性
                transforms.ToTensor(), # 转为Tensor
                normalize,  # 标准化
            ]))
        # 验证集：仅使用确定性变换（无随机增强）保证评估一致性，先缩放到256x256再中心裁剪，避免图像形变
        val_dataset = datasets.ImageFolder(
            valdir,
            transforms.Compose([
                transforms.Resize(256), # 等比缩放至短边256
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 如果是主进程（或非多进程分布式训练），保存检查点[PS： 暂时注释掉]

...
def image_processing_pipeline(data_dir,crop_size_x,crop_size_y):
    ...
    # images = fn.decoders.image_random_crop(images.gpu(), device="gpu", output_type=types.RGB)

    # the rest of processing happens on the GPU as well
    images = fn.resize(images.gpu(), resize_x=crop_size_x, resize_y=crop_size_y)
...
#加载数据集合的关键代码-------------------
        ...
        val_data = DALIGenericIterator(
            [image_processing_pipeline(val_dir, 244,244,batch_size=args.batch_size,num_threads=args.workers)], ['images', 'target'],
            reader_name='Reader'
        )
        # 检查流水线属性是否生效

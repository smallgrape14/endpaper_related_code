...
Open_DEBUG =1
CPU_affinity=0 # 是否开启 让进程运行在和GPU亲和的6个核心上
single_core=2 # 1表示为每个dataloader 指定单个core ,2 表示指定一个 GPU亲和的corelist 
all_core=1 # 表示使用全部的CPU核心，但是核心列表中
Open_flameprof=1 # 生成火焰图分析
import cProfile
...
def set_cpu_affinity(gpu_id, ngpus_per_node):
    # 假设每个 GPU 绑定 4 个 CPU 核心（可根据实际情况调整）
    if all_core == 1:
        cpu_cores_per_gpu = 40
    else:
        cpu_cores_per_gpu = 6
    # total_cores = os.cpu_count()
    # start_core = (gpu_id * cpu_cores_per_gpu) % total_cores
    # end_core = start_core + cpu_cores_per_gpu - 1
    # affinity = list(range(start_core, end_core + 1))
    affinity=[]
    
    if single_core == 2:
        start_core = gpu_id
        affinity.append(start_core) 
        for i in range(1,cpu_cores_per_gpu):
            affinity.append(start_core+2*i)
    elif single_core == 1:
        # 指定单一核心
        if gpu_id== 0:
            affinity.append(12)
    ...
    print(f"GPU {gpu_id} bound to CPU cores: {affinity}")

from functools import partial

def worker_set_affinity(gpu_id, worker_id):
    # 假设每个 GPU 绑定 4 个 CPU 核心（可根据实际情况调整）
    if all_core == 1:
        cpu_cores_per_gpu = 40
    else:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.gpu = gpu
    if CPU_affinity == 1:
        worker_set_affinity(args.gpu,ngpus_per_node)
    ...
    if args.workers > 0:
        if set_cpu_affinity == 1:
            train_loader = DataLoader(train_dataset,
                ...
                # worker_init_fn=lambda worker_id: set_cpu_affinity(gpu, ngpus_per_node),
                worker_init_fn = partial(worker_set_affinity, gpu_id=gpu),
                drop_last=True)
        else:
            train_loader = DataLoader(train_dataset,
                batch_size=args.batch_size, 
        ...
        if set_cpu_affinity == 1:
            train_loader = DataLoader(train_dataset,
                ...
                # prefetch_factor=1,
                # persistent_workers=True, # 持久化工作线程
                # worker_init_fn=lambda worker_id: set_cpu_affinity(gpu, ngpus_per_node), # 这个会报错
                worker_init_fn = partial(worker_set_affinity, gpu_id=gpu),
                drop_last=True)
        else:
            train_loader = DataLoader(train_dataset,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, criterion, optimizer, epoch, device, args,batch_data_list, batch_train_list,epoch,gpu)
...
def train(train_loader, model, criterion, optimizer, epoch, device, args,batch_data_list, batch_train_list,epoch_idx,gpu_id):
    ...
    if epoch_idx== 2 and Open_flameprof == 1:
    ...
    for i, (images, target) in enumerate(train_loader): #遍历 train_loader 中的每个批次数据。enumerate 用于获取批次索引 i 和批次数据 (images, target)
        ...
        if epoch_idx == 2  and i == 0 and Open_flameprof == 1:
            profiler.disable()
            profiler.dump_stats(f"worker_{gpu_id}.prof")  # 按GPU编号保存

...
Open_NVTX=1 # 表示打开nvtx profiler 标记
Close_Validate=1 # 1表示关闭validate函数
...
def main():
    ...
    if device.type =='cuda':
        ...
        ngpus_per_node = args.gpu_num #torch.cuda.device_count()
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    batch_time = AverageMeter('Time', use_accel, ':3.6f', Summary.NONE)
    data_time = AverageMeter('Data', use_accel, ':3.6f', Summary.NONE)
    losses = AverageMeter('Loss', use_accel, ':.4e', Summary.NONE)
    top1 = AverageMeter('Acc@1', use_accel, ':6.4f', Summary.NONE)
    top5 = AverageMeter('Acc@5', use_accel, ':6.4f', Summary.NONE)
    ...
    model.train()
    batch_idx=0
    Total_File_num=len(train_loader) #[todo list]

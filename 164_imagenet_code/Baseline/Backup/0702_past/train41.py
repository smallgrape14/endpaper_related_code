...
if __name__ == "__main__":
    ...
    print(f"batch_size : {BATCH_SIZE},epoch_num: {EPOCH_NUM},worker_num : {WORKER_NUM},conn_mode : {Conn_Mode}")

    # 在训练开始之前，初始化 CSV 文件并写入表头
    if Conn_Mode==0:
        csv_file_path = "whole_epoch_time_decomp_NFS_0525.csv"
    elif Conn_Mode==1:  
        csv_file_path = "whole_epoch_time_decomp_BeeGFS_0609.csv"
    elif Conn_Mode==2:
    ...
    # 创建数据加载器
    batch_size = BATCH_SIZE
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,num_workers=WORKER_NUM,prefetch_factor=1)

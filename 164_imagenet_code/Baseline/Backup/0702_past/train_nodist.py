    ...
    def _read_file_infos(self):
        if Opt_Mode == 3 :
            ...
            for _ in range(self.file_num):
                ...
                label = int(label.get())
                offset += np.dtype(np.uint64).itemsize
                # print(f"{file_idx} th File : file_path= {file_path}; \t file_begin = {file_begin}; \t file_size = {file_size}; \t label = {label}")
...
def import_shared_memory_and_train(_batch_size=16):
    ...
    csv_file_path = "IPC/Train/GPUNetIO/result/epoch_time_decomp_0627.csv"
    # 检查文件是否存在并且是否有表头
    ...
    ## 每个batch平均值的记录文件
        # 在训练开始之前，初始化 CSV 文件并写入表头
    avg_csv_file_path = "IPC/Train/GPUNetIO/result/avg_batch_time_decomp_0627.csv"
    # 检查文件是否存在并且是否有表头
    ...
    if msg in list_data: #检查 msg 是否在 list_data 中
        ...
        num_classes =2 #len(dataset.num_classes) # todolist 增加目录类别时需要修改这个值 
        ...
        train_loader = DataLoader(dataset, batch_size=_batch_size, shuffle=False)
            ...
            if Open_nvtx == 0 :
                ...
                while(file_num < Total_File_Num):
                    ...
                    with nvtx.range("dataset"):
                        dataset = SharedMemoryDataset(ipc_handle_file_0,cur_batch_idx)
                        train_loader = DataLoader(dataset, batch_size=_batch_size, shuffle=False)#num_workers=4 pin_memory=True 这些参数都不能用

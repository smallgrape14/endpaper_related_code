...
def train_model(model, criterion, optimizer, train_loader,stat_file_path, num_epochs=10,prefetch_factor=0,):
    # 清理页面缓存、目录项和inode
    os.system('sync; echo 3 > /proc/sys/vm/drop_caches')
    # 初始化全局统计容器（存储每个batch的时间数据）
    all_epoch_data = {
        'total_times': [],
        'data_times': [],
        'data_whole_times': [],
    ...
    for epoch in range(num_epochs):
        ...
        # 更新全局统计
        all_epoch_data['data_times'].append(epoch_data_time)
        all_epoch_data['data_whole_times'].append(epoch_data_whole_time)
        all_epoch_data['train_times'].append(epoch_train_time)
        all_epoch_data['total_times'].append(epoch_total_time)

        all_stats = {k: calc_stats(v) for k, v in all_epoch_data.items() if v}
        print(f"\nEpoch {epoch} - Cumulative Statistics (All Epochs):")
        for metric in ['total_times', 'data_times', 'data_whole_times', 'train_times']:
            stats = all_stats.get(metric, {})
            print(f"  {metric.replace('_', ' ').title()}: "
                  f"Avg={stats.get('avg', 0):.6f}s, "
                  f"Min={stats.get('min', 0):.6f}s, "
                  f"Max={stats.get('max', 0):.6f}s, "
                  f"P99={stats.get('p99', 0):.6f}s")
        ...
        for metric, prefix in [('total_times', 'global_'), ('data_times', 'global_data_'), 
                                ('data_whole_times', 'global_data_whole_'), ('train_times', 'global_train_')]:
            stats = all_stats.get(metric, {})
            base_data.update({
                f"{prefix}avg": stats.get('avg', 0),
                f"{prefix}min": stats.get('min', 0),
                f"{prefix}max": stats.get('max', 0),
                f"{prefix}p99": stats.get('p99', 0)
            })
        with open(stat_file_path, mode='a', newline='') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
...
if __name__ == "__main__":
    ...
    # 多次重复测量，平均值/MAX/MIN/p99 统计结果
    stat_file_path="result/0617/statatisc_epoch_time_BeeGFS_0617.csv"
    # 检查文件是否存在并且是否有表头
    write_header = not os.path.exists(stat_file_path)
    if not write_header:
        with open(stat_file_path, mode='r', newline='') as csv_file:
            reader = csv.reader(csv_file)
            try:
                header = next(reader)
                write_header = len(header) == 0 or header[0] != 'batch_size'
    ...
    model = train_model(model, criterion, optimizer, train_loader,stat_file_path=stat_file_path, num_epochs=EPOCH_NUM,prefetch_factor=prefetch)

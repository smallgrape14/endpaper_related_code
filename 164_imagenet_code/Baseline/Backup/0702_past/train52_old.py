...
Open_NVTX=0
Open_acc1_2=0 # 是否打开 ACC1 和 ACC2 的计算
Clean_Cache=0 # 是否清理缓存
Modify_Train_End= 0 # 调整train end 的时间戳位置
...
def train_model(model, criterion, optimizer, train_loader, num_epochs=10,prefetch_factor=0):
    ...
    for epoch in range(num_epochs):
        ...
        if Open_NVTX==1:
            ...
            for inputs, labels in train_loader:
                ...
                if Open_acc1_2==1:
                    ...
                    print(f"acc1: {acc1.item()}, acc2: {acc2.item()}")
        ...
        if Open_acc1_2 == 1:
            acc1_avg = np.mean(acc1_list)
            acc2_avg = np.mean(acc2_list)
            print(f'epoch {epoch} Train Loss: {epoch_loss:.6f} Acc: {epoch_acc:.6f},Acc1: {acc1_avg:.6f}, acc2: {acc2_avg:.6f}, data time: {epoch_data_time:.6f}, data whole time: {epoch_data_whole_time:.6f},train time: {epoch_train_time:.6f},total_time: {epoch_total_time:.6f}')
        else:

...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if gpu == 0: # 只在主进程（GPU 0）写入统计结果
        if Open_DEBUG == 1:
            avg_csv_file_path = "result/dist/avg_batch_time_dist_debug_b{args.batchsize}_w{args.workers}.csv"
        else: 
            avg_csv_file_path = f"result/dist/avg_batch_time_dist_b{args.batchsize}_w{args.workers}.csv"
        write_header = not os.path.exists(avg_csv_file_path)
        with open(avg_csv_file_path, mode='a', newline='') as csv_file:

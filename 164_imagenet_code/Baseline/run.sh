...
do
    ...
    do
        for worker_num in 4 #64 128 256 #256  #32 64 #128  #32 40 64 128
        do  
            for prefetch_factor in 1 #2 3 4
            do
                # 执行 Python 脚本并传递参数
                # sudo python3 $PYTHON_SCRIPT --batch_size $BATCH_SIZE --epochs $EPOCH_NUM --workers $worker_num
                sudo python3 $PYTHON_SCRIPT --batch_size $batch_size --epochs $EPOCH_NUM --workers $worker_num --conn_mode $CONN_MODE --prefetch $prefetch_factor --gpu $gpu_id #> 0806_B128_W32_2class.txt
                # echo "已完成 worker_num = $worker_num"
            done
        done
    done
...
# for batch_size in 1024 16 32 64 128 256 512 
# do
#     for worker_num in 16 #0 4 8 16 32 #0 8 32 40 #0 4 8 16 32 64 
#     do  

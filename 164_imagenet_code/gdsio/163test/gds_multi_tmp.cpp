...
*/
bool measureReadOperationOnly(const string& file_path, FileResources& res, size_t iosize) {
...
int main(int argc, char *argv[]) {
    ...
    stats.total_bytes=files.size()*io_size;
    stats.files_processed=files.size();
    stats.total_operations=files.size();
    ...
    for (const auto& file : files) {
        if (file_resources.find(file.path) != file_resources.end()) {
                ...
                bool  success = measureReadOperationOnly(file.path, res, io_size);

...
#define vary_iosize 1 //1表示启用iosize参数
...
public:
    ...
    void error(const string& message) {
        lock_guard<mutex> lock(log_mutex);
        cerr << "[ERROR Thread " << this_thread::get_id() << "] " << message << endl;
    }
};
...
FilePerfStats measureReadOperationOnly(const string& file_path, FileResources& res, size_t iosize) {
    ...
    // cudaError_t cuda_status = cudaSetDevice(device_id);
    // if (cuda_status != cudaSuccess) {
    //     cerr << "线程设置CUDA设备失败: " << cudaGetErrorString(cuda_status) << "，设备ID: " << device_id << endl;
    //     return file_stats;
    // }
    ...
    // 测量读操作
    for (size_t offset = 0; offset < res.file_size; offset += iosize) {
        size_t read_size;
        #if vary_iosize == 1
            read_size = iosize;
        #else
        ...
        cout << "读取偏移量 " << offset << " 文件: " << file_path << " iosize: " << read_size <<"elapsed_time: " <<elapsed_time<<endl;
...
// 计算性能指标
void calculatePerformanceMetrics(PerfStats& stats, int64_t total_duration_ms) {
    // if (total_duration_ms > 0) {
        double total_time_seconds = total_duration_ms / 1000.0;
...
bool writeSummaryToCSV(const PerfStats& stats, const string& csv_filename, 
                      const string& directory_path, const string& iosize, 
                      int thread_count, int64_t duration_ms) {
...
int main(int argc, char *argv[]) {
    ...
    cout << "开始多线程读操作测试，使用 " << thread_count << " 个线程..." << endl;
    ...
    for (const auto& file : files) {
        if (file_resources.find(file.path) != file_resources.end()) {
            ...
            futures.emplace_back(
                pool.enqueue([&res, &file, io_size, &logger]() -> FilePerfStats {
                    logger.log("开始处理文件: " + file.name);
                    
                    // 执行读取操作并测量性能
                    auto file_stats = measureReadOperationOnly(file.path, res, io_size);
                    
                    if (file_stats.success) {
                        logger.log("完成: " + file.name + " (" + 
                                 to_string(file_stats.bytes_read / 1024) + " KB, " + 
                                 to_string(file_stats.read_time_ms) + " ms)");
                    } else {
                        logger.error("失败: " + file.name);
                    }
    ...
    cout << "已提交 " << futures.size() << " 个任务到线程池" << endl;
    
    // 等待所有任务完成并收集结果
    cout << "等待所有任务完成..." << endl;
    for (auto& future : futures) {
        FilePerfStats file_stats = future.get(); // 阻塞直到任务完成
    ...
    cout << "使用线程数: " << thread_count << endl;
    cout << "IOSIZE: " << io_size_str << " (" << io_size << " 字节)" << endl;
    cout << "资源预分配耗时: " << prealloc_duration.count() << " ms" << endl;
    cout << "总执行时间（包含线程开销）: " << duration.count() << " ms" << endl;
    // cout << "GPU读操作总时间: " << stats.total_time_ms << " ms" << endl;
    ...
    cout << "多线程加速比: " << (stats.total_time_ms / (double)duration.count()) << endl;
    cout << "===============================================" << endl;

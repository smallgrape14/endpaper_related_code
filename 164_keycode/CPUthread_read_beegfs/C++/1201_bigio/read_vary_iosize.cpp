...
*/
//读取文件大小是IOSIZE 而非文件本身的大小
void process_files(const std::string& directory_path, const std::map<std::string, int>& file_descriptors, 
                   const std::map<std::string, off_t>& file_sizes, int thread_id, int start, int end,int iosize) {
    std::string full_path;
    ...
    for (int i = start; i < end; ++i) {
        ...
        // }
        // off_t file_size = file_size_it->second;
        off_t file_size = iosize*1024;//iosize单位是KB 需要转换为Byte
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cerr << "用法: " << argv[0] << "<directory>  <thread_num>  <IO size>" << std::endl;
        return 1;
    }
    ...
    int IOSIZE = stoi(argv[2]); // IOSize 单位是KB
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        //                     std::cref(file_sizes), i + 1, start, end);
         threads.emplace_back(process_files, directory_path, std::cref(file_descriptors), 
                            std::cref(file_sizes), i + 1, start, end,IOSIZE);
        start = end;
    }
    ...
    // 将结果写入 CSV 文件 
    #if US_OPEN == 0 
    std::ofstream csv_file("read_results_0425_1MB_vary_iosize_s.csv", std::ios::app);//@@ TODO
    #elif US_OPEN == 1 
        std::ofstream csv_file("read_results_0425_1MB_vary_iosize_us.csv", std::ios::app);//@@ TODO
    #endif
    if (csv_file.is_open()) {
        if (csv_file.tellp() == 0) {
            #if US_OPEN == 0 
                csv_file << "dir_name,thread_num,iosize(KB),total_read_time(ms),total_read_time(s)," << std::endl;
            #elif US_OPEN == 1
                csv_file << "dir_name,thread_num,iosize(KB),total_read_time(us),total_read_time(ms)," << std::endl;
            #endif
        }
        #if US_OPEN == 0 
            csv_file << directory_path << "," << num_threads <<","<<IOSIZE<< "," << total_time << "," << total_time / 1000.0 << std::endl;
        #elif US_OPEN == 1
            csv_file << directory_path << "," << num_threads<<","<<IOSIZE << "," << total_time_us << "," << total_time_ms << std::endl;
        #endif
        csv_file.close();
    } else {

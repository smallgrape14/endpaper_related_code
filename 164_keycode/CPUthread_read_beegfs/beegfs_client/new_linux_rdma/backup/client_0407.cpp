...
#define MALF_SIZE 64
#define D_BUFSIZE 340*1024*1024
int total_recv=0;
struct context
{
...
void build_qp_attr(struct ibv_qp_init_attr *qp_attr)
{
  ...
  qp_attr->cap.max_recv_wr = FILE_NUM * 2;
  qp_attr->cap.max_send_sge = 16; // 支持多SGE操作
  qp_attr->cap.max_recv_sge = 16;
  qp_attr->cap.max_inline_data = 128; //// 内联数据传输优化
}
...
void register_memory(struct connection *conn)
{
  ...
  for (int i = 0; i < 256; i++)
  {
    conn->recv_mr_pool[i] = ibv_reg_mr(s_ctx->pd, conn->recv_pool[i], MALF_SIZE, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
  }
  ...
  for (int i = 0; i < 256; i++)
  {
    conn->send_mr_pool[i] = ibv_reg_mr(s_ctx->pd, conn->send_pool[i], MAX_FILE_SIZE, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
  }
...
void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode & IBV_WC_RECV)
  {
    total_recv++;
    // 接收到服务端完成通知
    printf("[on_completion]已接收到 %d  recv \n",total_recv);
  }
...
int on_connection(void *context)
{
  ...
  printf("vaddr : 0x%08x,  rkey : 0x%016llx \n",vaddr,rkey);

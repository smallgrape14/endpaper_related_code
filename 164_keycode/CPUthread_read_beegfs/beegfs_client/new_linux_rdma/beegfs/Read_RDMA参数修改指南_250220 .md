...
//重新编译后，一般会更新 targetID 需要再下面设置新的 TargetID
4. new_read.cpp:  
    Raid0_StripePattern_init()
    {
        ...
        //根据存储目标的数量，填充 targetNumID 数组 ，将存储目标的ID 一一填充到该数组
         targetNumID[0]=3001;//@@ 存储 Target ID //获取指令： sudo beegfs-ctl --listtargets --longnodes ; ex 输出 :
                                        //TargetID   NodeID
                                        //========   ======
...
GPUNetIO 的应用还需要改如下：
1. gpunetio_rdma_client_server_write/beegfs/getentryid.hpp
    read_dir()
    {
            unsigned int origParentUID=1000;//上帝视角的数据设置 父目录的userID // 获取指令 stat /mnt/beegfs/test_250220 

    }
2. gpunetio_rdma_client_server_write/beegfs/new_read.hpp
    generate_read_request()
    {
        uint16_t targetID=3001;//@@TODO sudo beegfs-ctl --listtargets --nodetype=storage --state
    }
```
 ...
 __MessagingTk_requestResponseWithRRArgsComm()
{
        ...
        localNodeNumID.value=3;//@@ client NodeID //获取指令： sudo beegfs-ctl --listnodes --nodetype=client ;ex 输出
        ...
        {   
            metaOwnerInit(&rootOwner);
            {
                ...
                rootOwner->node.value=200;//@@ 元数据节点ID //获取指令： sudo beegfs-ctl --listnodes --nodetype=meta ;ex 输出
...
2. new_read.cpp:  
    Raid0_StripePattern_init()
    {
        int targetnum=1;//@@ 存储目标有几个，默认是 1个 //查询指令： sudo beegfs-ctl --listnodes --nodetype=storage ; 输出有几个条目就是几个
        //根据存储目标的数量，填充 targetNumID 数组 ，将存储目标的ID 一一填充到该数组
         targetNumID[0]=301;//@@ 存储 Target ID //获取指令： sudo beegfs-ctl --listtargets --longnodes ; ex 输出 :
                                        //TargetID   NodeID
                                        //========   ======
                                        //61   beegfs-storage ubuntu-PowerEdge-R750xa [ID: 6]  
        targetNumID[1]=4;//@@
...
NodeStoreEx_referenceNodeByTargetID () //这个函数需要初始化 存储节点  Node 节点的信息
{
     //连targetid 都不用传
        NumNodeID nodeID;
        nodeID.value=300;//@@ 存储Node_ID //获取指令： sudo beegfs-ctl --listnodes --nodetype=storage ; ex 输出 : ubuntu-PowerEdge-R750xa [ID: 6] 
        // 假设的节点类型、ID 和端口
            ...
            uint16_t nodenumID = 300; //@@ 存储Node_ID //获取指令： sudo beegfs-ctl --listnodes --nodetype=storage ; ex 输出 : ubuntu-PowerEdge-R750xa [ID: 6] 
...
{
    NumNodeID localNodeNumID ;
    localNodeNumID.value=3; //@@ client Node ID//获取指令： sudo beegfs-ctl --listnodes --nodetype=client ;ex 输出
                                                //     128A1B-67B56FCE-node9 [ID: 5] //164client
                                                // 182CF0-67B57038-ubuntu-PowerEdge-R750xa [ID: 6] //162client
}
6. read.cpp
...
## 250303 补充 generate RDMA Read Request MSG

```sh
# 生成消息请求的代码在 new_read.hpp 中 
__commkit_readfile_prepareHeader
```

...
该版本的代码时 合并了 readdir 和 rdma_read 模块的最终代码

```sh
# 原来结构
readfilevec ==>  new_read  ==> read_pre ==> RDMAConnection ==> (1) open (2) IBVSocket

getentryid ==> readdir  ==> new_read ==> open

1. readfilevec 被我去掉了，其中的函数功能全转移到new_read中了
2. 对比 open.hpp ，保留 RDMA Read 的功能，将 readdir 的增加的功能添加到 RDMA_READ 中的 open.hpp
3. 对比 new_read , 保留两个pathinfo_init的函数，保留两个 initRemotingIOInfo 函数 ，保留 readdir 的 __commkit_prepare_generic 这个准备read消息的函数/ FhgfsOpsCommkit_communicate 
/ FhgfsOpsCommKit_readfileV2bCommunicate/FhgfsOpsRemoting_readfileVec
# 新的结构
                             new_read  ==> read_pre ==> RDMAConnection ==> (1) open (2) IBVSocket
...
新增加了 RDMA open 的逻辑，函数实现open_RDMA 位于 new_read.hpp 

...
static RdmaInfo * RdmaInfo_map(const struct iovec *iter, Socket *socket,enum dma_data_direction dma_dir,uint64_t vaddr,uint32_t rkey,size_t memory_len);//XYP_MODIFY

// RdmaInfo* RdmaInfo_mapRead(const struct iov_iter *iter, Socket *socket)
RdmaInfo* RdmaInfo_mapRead(const struct iovec *iter, Socket *socket,uint64_t vaddr,uint32_t rkey,size_t memory_len)//XYP_MODIFY
{
   return RdmaInfo_map(iter, socket, DMA_FROM_DEVICE,vaddr,rkey,memory_len);
}
...
{
   LOG(DEBUG)<<"sock->commCfg.keyType = "<<sock->commCfg.keyType<<"\n";
...
int nvfs_dma_map_sg_attrs( user_scatterlist *sglist, int sg_count) 
{
   ...
   //  }
    ...
    LOG_printf(DEBUG,"Total length to be mapped: %lu\n", total_length);
...
static RdmaInfo * RdmaInfo_map(const struct iovec *iter, Socket *socket,enum dma_data_direction dma_dir,uint64_t vaddr,uint32_t rkey,size_t memory_len)//XYP_MODIFY
{
   //@@12_9
      ...
      //XYP_MODIFY
      size_t totalSize =memory_len;//iter->iov_len;
...
/* //XYP_MODIFY 注释下面内容
      ...
      cout << "\n";
      LOG_printf(DEBUG,"buffer printf = %s\n", totalbuffer);
*/
...
RDMASocket* rs = (RDMASocket*)socket;
    //XYP_MODIFY 注释下面内容
      ...
      //XYP_MODIFY
      rdmap0->key = rkey;//mr->rkey; // or mr->lkey，根据你的使用场景
      LOG_printf(DEBUG,"rdmap0->key = %d\n", rdmap0->key);



      // LOG(DEBUG)<<"mr->rkey ="<<mr->rkey<<"\n";
      LOG_printf(DEBUG,"--------1------------\n");
      // XYP_MODIFY 250303
      ...
      rdmap0->device = ib_dev;//rs->ibvsock.commContext->pd->context->device;
      LOG_printf(DEBUG,"--------2------------\n");
      ...
      //XYP_MODIFY
      rdmap0->dmalist[0].dma_address =vaddr;// (dma_addr_t)(uintptr_t)totalbuffer;
      ...
      //XYP_MODIFY
      rdmap0->sglist[0].dma_address = vaddr;//(dma_addr_t)(uintptr_t)totalbuffer;
...
static RdmaInfo * RdmaInfo_map(const struct iovec *iter, Socket *socket,enum dma_data_direction dma_dir)
{
   ...
   unsigned    npages;
   unsigned    key;
   LOG(DEBUG)<<"Socket_getSockType(socket) = "<<Socket_getSockType(socket)<<"\n";
   ...
   if (!RDMASocket_isRkeyGlobal(rs))
   {
      ...
      LOG_printf(DEBUG,"ERROR: rkey type is not compatible with GDS\n");
   ...
   if (!rdmap || !sglist || !dmalist)
   {
      ...
      LOG_printf(DEBUG,"%s: no memory for scatterlist\n", __func__);
      free(rdmap);
        free(sglist);
        free(dmalist);
      status = -ENOMEM;
   ...
   LOG(DEBUG)<<"sg_count = "<<sg_count<<"\n";
   if (sg_count < 0)
   {
      // printk_fhgfs(KERN_ERR, "%s: can't convert iov_iter to scatterlist\n", __func__);
      LOG_printf(DEBUG,"%s: can't convert iov_iter to scatterlist\n", __func__);
   ...
   LOG(DEBUG)<<"count = "<<count <<"\t sg_count = "<<sg_count<<"\n";
   if (count != sg_count)
   {
      LOG(DEBUG)<<"count != sg_count \n";
      if (count == NVFS_CPU_REQ)
      {
         // printk_fhgfs(KERN_ERR, "%s: NVFS_CPU_REQ\n", __func__);
         LOG_printf(DEBUG,"%s: NVFS_CPU_REQ\n", __func__);

         status = 0;
      }
      else if (count == NVFS_IO_ERR)
      {
         LOG_printf(DEBUG,"%s: can't DMA map mixed CPU/GPU pages\n", __func__);
         status = -EINVAL;
      }
      else
      {
         LOG_printf(DEBUG,"%s: unknown error returned from NVFS (%d)\n", __func__, count);
         status = -EIO;
      }
      goto error_return;
   }
   ...
   if (dma_count > RDMA_MAX_DMA_COUNT)//>64
   {
      ...
      LOG_printf(DEBUG,"%s: too many DMA elements count=%d max=%d\n", __func__,
         dma_count, RDMA_MAX_DMA_COUNT);
      status = -EIO;
      goto error_return;
   }

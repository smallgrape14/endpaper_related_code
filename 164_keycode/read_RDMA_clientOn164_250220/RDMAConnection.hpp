...
bool __IBVSocket_createNewID(IBVSocket* _this)
{
   ...
   // if(IS_ERR(new_cm_id) )
   // {
   //    printf("rdma_create_id failed. ErrCode: %ld\n", PTR_ERR(new_cm_id) );
   //    return false;
   // }
...
{
   printf("for IBVBuffer_fill\n");
   ssize_t total = 0;
   unsigned i;
   std::cout<<"buffer->bufferCount  = "<<buffer->bufferCount <<"\t";
   std::cout<<"iovec_iter_count(iter) = "<<iovec_iter_count(iter)<<"\n";
   for(i = 0; i < buffer->bufferCount && iovec_iter_count(iter) > 0; i++)
   {
      ...
      total += fragment;
      printf("for i=%d, buffer data[%d]: %s\n", i, i,buffer->buffers[i]);
      std::cout<<"fragment = "<<fragment<<"\n";
      for(int j=0;j<fragment;j++)
      {
         // std::cout<<"j = "<<j <<"\n";
         std::cout<<buffer->buffers[i][j];
      }
      std::cout<<"--------\n";
      buffer->datasize[i]=fragment;//NOTE:@@
      std::cout<<"buffer->datasize = "<<buffer->datasize<<"\n";//NOTE:@@
      std::cout<<"\n";
      std::cout<<"strlen(buffer) = "<<strlen(buffer->buffers[i])<<"\n";
      std::cout<<" bufferSize  = "<<buffer->bufferSize<<" \t,bufferCount = "<<buffer->bufferCount<<"\t,listLength="<<buffer->listLength<<"\n";

   }

   printf("IBVBuffer_fill over\n");
   return total;
}
   ...
   IBVCommConfig* commCfg, IBVCommContext** outCommContext)
{
   ...
   fprintf(stderr, "Alloc CommContext @ %p\n", commContext);
   ...
   std::cout<<"#### commCfg->bufNum = "<<commCfg->bufNum<<"\t"
   ...
   for(i=0; i < commCfg->bufNum; i++)
   {
      ...
      if(recvBufRes){
         // printf("registration recvBufs[%d] successfully!\n", i);
      }
      else{
         printf("registration recvBufs[%d] failed!\n", i);
         goto err_cleanup;
      }
   }
   ...
   for(i=0; i < commCfg->bufNum; i++)
   {
      ...
      if(recvBufRes){
         // printf("registration sendBufs[%d] successfully!\n", i);
      }
      else{
         printf("registration sendBufs[%d] failed!\n", i);
         goto err_cleanup;
      }
   }
...
printf("try rdma_connect\n");
   int conRes = rdma_connect(_this->cm_id, &conn_param);
   cout << "rdma_connecr res = " << conRes << "\n";
   return conRes;
}
...
bool __IBVSocket_parseCommDest(const void* buf, size_t bufLen, IBVCommDest** outDest)
{
   ...
   {
      printf("Bad private data size. length: %zu\n", bufLen);
      return false;
   }
...
{  
   // std::cout<<" into  __IBVSocket_postRecv \n";
   ...
   // std::cout<<" prepare into  ibv_post_recv \n";

   postRes = ibv_post_recv(commContext->qp, &wr, &bad_wr);
   // std::cout<<" out of  ibv_post_recv, postRes = "<< postRes<< "\n";

   if(unlikely(postRes) )
   {
      printf("ib_post_recv failed. ErrCode: %d\n", postRes);
...
int __IBVSocket_connectedHandler(IBVSocket* _this, struct rdma_cm_event* event)
{
   ...
   for(i=0; i < commCfg->bufNum; i++)
   {
      ...
      {
         printf("couldn't post recv buffer with index %d\n", i);
...
int __IBVSocket_cmaHandler(struct rdma_cm_id* cm_id, struct rdma_cm_event* event)
{
   ...
   fprintf(stderr, "rdma event: %i, status: %i\n", event->event, event->status);
   ...
   switch(event->event)
   {
      ...
      case RDMA_CM_EVENT_DEVICE_REMOVAL:
          ...
          */
         printf("Device has been removed: %s\n", cm_id->verbs->device->name);
         break;
...
if (pthread_create(&event_thread, NULL, rdma_event_loop, (void*)_this)) {
   ...
   for( ; ; ) // stale retry loop 失败重试
   {
      ...
      printf("解析地址并为RDMA连接创建了通信标识符cm_id\n");
      ...
      // 释放互斥锁
      pthread_mutex_unlock(&_this->eventMutex);
      printf("connState after pthread_cond_timedwait %u\n", _this->connState);
      ...
      if(_this->connState == IBVSOCKETCONNSTATE_REJECTED_STALE)
      {
         ...
         { // no more stale retries left
            if(IBVSOCKET_STALE_RETRIES_NUM) // did we have any retries at all
               printf("Giving up after %d stale connection retries\n",
...
{
   std::cout<<"go into __IBVSocket_waitForRecvCompletionEvent \n";
   IBVCommContext* commContext = _this->commContext;
   long waitRes;
   int numEvents = 0;
   int checkRes;
   ...
   std::cout<<"timeoutMS = "<<timeoutMS<<"\n";
   std::cout<<"outWC->status = "<<outWC->status<<"\n";
   if(!timeoutMS)
   ...
   while(timeoutMS != 0)
   {
      ...
      // long pollTimeoutJiffies = TimeTk_msToJiffiesSchedulable(pollTimeoutMS);  // 用户空间不需要转化为节拍，直接用ms
      ...
      //    (numEvents = ibv_poll_cq(commContext->recvCQ, 1, outWC) ), pollTimeoutJiffies);  // 等待事件完成或直到超期

      // // if(unlikely(waitRes == -EINTR || fatal_signal_pending(current)))
      // { // signal pending
      //    fprintf(stderr, "wait for recvCompEvent ended by pending signal\n");

      //    return waitRes;
      // }
      cnt++;
      printf("------------%d--------------\n",cnt);
      std::cout<<"pollTimeoutMS = "<<pollTimeoutMS<<"\n";
      ...
      if(likely(numEvents) )
      { // we got something
         std::cout<<" we got something \n";
         return numEvents;
      }

      // timeout
      std::cout<<"prepare go into IBVSocket_checkConnection \n";
      checkRes = IBVSocket_checkConnection(_this);//$$$% 因为这里发送包 ，我把这个先注释了
      std::cout<<"checkRes = "<<checkRes<<"\n";
      if(checkRes < 0)
         return -ECONNRESET;
...
{
   std::cout<<" start __IBVSocket_recvWC \n";
   IBVCommContext* commContext = _this->commContext;
   int waitRes;
   size_t bufIndex;
   std::cout<<" prepare go into  __IBVSocket_waitForRecvCompletionEvent \n";
   std::cout<<"outWC->status = "<<outWC->status<<"\n";

   waitRes = __IBVSocket_waitForRecvCompletionEvent(_this, timeoutMS, outWC);
   std::cout<<"waitRes ="<<waitRes<<"\n";
   if(waitRes <= 0)
...
int __IBVSocket_flowControlOnSendWait(IBVSocket* _this, int timeoutMS)
{
   ...
   std::cout << "numSendLeft " << commContext->numSendBufsLeft << "\n";
   if(commContext->numSendBufsLeft)
   {
      std::cout<<"// flow control not triggered yet 发送缓冲区未满，不需要控制 \n";
   ...
   unsigned* numSendElements, unsigned* numWriteElements, unsigned* numReadElements, int timeoutMS)
{
   ...
   do
   {
      ...
      cout << "oldSendout: " << oldSendCount << "\n";

      numElements = ibv_poll_cq(commContext->sendCQ, 2, wc); // 轮询发送完成队列，最多获取两个工作完成（wc）
      printf("__IBVSocket_waitForTotalSendCompletion ibv_pool_cq returns %d\n", numElements);
      ...
      { // no completions available yet => wait
         ...
         printf("in __IBVSocket_waitForTotalSendCompletion : __IBVSocket_waitForSendCompletionEvent returns %d\n", waitRes);
         if(likely(waitRes > 0) )
            continue;
...
{
   std::cout<<" go into __IBVSocket_postSend \n";
   IBVCommContext* commContext = _this->commContext;
   /*
   //以下是修改前的代码 wsy_source_code
      ...
      wr.sg_list    = commContext->sendBufs[bufIndex].lists;
            ...
            std::cout<<"list.addr = "<<commContext->sendBufs[bufIndex].lists->addr<<"\t ,list.length ="<<commContext->sendBufs[bufIndex].lists->length
            <<"\t, list.lkey = "<<commContext->sendBufs[bufIndex].lists->lkey<<"\n";
            for(int i=0;i<48;i++)
            {
               std::cout<<reinterpret_cast<char*>(commContext->sendBufs[bufIndex].lists->addr)[i];
            }
            std::cout<<"\n lists.addr \n"; //这个内容是正确的
...
//   struct ibv_send_wr wr;
   ...
   //          }
   //          std::cout<<"\n lists.addr \n"; //这个内容是正确的
   //          std::cout<<"strlen = "<<strlen(reinterpret_cast<char*>(commContext->sendBufs[bufIndex].lists->addr))<<"\n";
   size_t buffer_size=commContext->sendBufs[bufIndex].datasize[0];
   std::cout<<" commContext->sendBufs[bufIndex].datasize[0] , buffer_size = "<<buffer_size<<"\n";
   char *testbuffer = (char *)malloc(buffer_size);
   if (!testbuffer) {
      fprintf(stderr, "Failed to allocate memory\n");
      return -1;
   }
   std::cout<<"Successfully to allocate memory\n ";
   ...
   {
      // std::cout<<commContext->sendBufs[bufIndex].buffers[0][j];
      std::cout<<testbuffer[j];
   ...
   std::cout<<" out of __IBVSocket_postSend \n";
...
int __IBVSocket_nonblockingSendCheck(IBVSocket* _this)
{
   ...
   printf("in __IBVSocket_nonblockingSendCheck: __IBVSocket_flowControlOnSendWait returns %d \n", flowControlRes);
   ...
   printf("--------------onblockingSendCheck----------- \n");
   struct IBVIncompleteSend send = _this->commContext->incompleteSend;
   std::cout << "IBVIncompleteSend:\n";
   std::cout << "  numAvailable: " << send.numAvailable << "\n";
   std::cout << "  forceWaitForAll: " << (send.forceWaitForAll ? "true" : "false") << "\n";
   std::cout << "commCfg.bufNum"<< commContext->commCfg.bufNum << "\n";
   ...
   waitRes = __IBVSocket_waitForTotalSendCompletion(_this,
      &commContext->incompleteSend.numAvailable, &numWaitWrites, &numWaitReads, timeoutMS);
   printf("in __IBVSocket_nonblockingSendCheck : __IBVSocket_waitForTotalSendCompletion returns %d \n", waitRes);
   if(unlikely(waitRes < 0) )
      goto err_invalidateSock;
...
{
   printf("IBVSocket_senf begin. list iter data\n");
   for(int i=0;i<iter->iovcnt;i++){
      printf("%s\n", iter->iov[i].iov_base);
   }
   std::cout<<"iter = \n";
   std::cout<<"iter->total_len = "<<iter->total_len<<"\n";
   for(int i=0;i<iter->total_len;i++){
   std::cout<<static_cast<char*>(iter->iov[0].iov_base)[i];
   }
   std::cout<<"\n";
   IBVCommContext* commContext = _this->commContext;   // 通信上下文
  
   struct IBVIncompleteRecv recv = _this->commContext->incompleteRecv;
   std::cout << "IBVIncompleteRecv:\n";
   std::cout << "  isAvailable: " << recv.isAvailable << "\n";
   std::cout << "  completedOffset: " << recv.completedOffset << "\n";
   std::cout << "  bufIndex: " << recv.bufIndex << "\n";
   std::cout << "  totalSize: " << recv.totalSize << "\n";
   struct IBVIncompleteSend send = _this->commContext->incompleteSend;
   std::cout << "IBVIncompleteSend:\n";
   std::cout << "  numAvailable: " << send.numAvailable << "\n";
   std::cout << "  forceWaitForAll: " << (send.forceWaitForAll ? "true" : "false") << "\n";
   ...
   { // send only as much as we can without blocking
      ...
      printf("in IBVSocket_send: __IBVSocket_nonblockingSendCheck returns %d \n", checkSendRes);
      if(!checkSendRes)
   ...
   do  //将数据按照缓冲区大小分片发送
   {
      ...
      for(int j=0;j<48;j++)
      {
         std::cout<<commContext->sendBufs[currentBufIndex].buffers[0][j];
      }
      ...
      postRes = __IBVSocket_postSend(_this, currentBufIndex);
      std::cout<<"after __IBVSocket_postSend, postRes ="<<postRes<<"\n";
      if(unlikely(postRes) )
      {
         commContext->incompleteSend.numAvailable--;
         goto err_invalidateSock;
...
int __IBVSocket_receiveCheck(IBVSocket* _this, int timeoutMS)
{
   ...
   std::cout<<"flowControlRes =  "<<flowControlRes<<"\n";
   if(unlikely(flowControlRes < 0) )
   {
   ...
   std::cout<<"recv one packet (if available) and add it as incompleteRecv \n";
   recvRes = __IBVSocket_recvWC(_this, timeoutMS, &wc);
...
{
   // std::cout<<"go into __IBVSocket_recvContinueIncomplete \n";
   IBVCommContext* commContext = _this->commContext;
   ...
   size_t copyRes = 0;
   ssize_t total = 0;
   // std::cout<<"_this->errState = "<<_this->errState<<"\n";
   // std::cout<<"recv->completedOffset = "<<recv->completedOffset<<"\n";
   // std::cout<<"recv->totalSize = "<<recv->totalSize<<"\n";
   if(unlikely(_this->errState) )//$$%
      return -1;
   ...
   while(iovec_iter_count(iter) > 0 && recv->totalSize != recv->completedOffset)
   {
      ...
      // for(int j=0;j<fragment;j++)
      // {
      //    std::cout<<std::hex << std::setw(2) << std::setfill('0') << (int)(buffer->buffers[page] + offset)[j]<<" ";
      // }
      // std::cout << std::dec << std::endl; // 打印完毕后切换回十进制格式
      // std::cout<<"\n buffer end \n";
   ...
   if(recv->completedOffset == recv->totalSize)
   {
      ...
      // std::cout<<" prepare go into __IBVSocket_postRecv \n ";
      postRes = __IBVSocket_postRecv(_this, _this->commContext, bufIndex);
      // std::cout<<" out of  __IBVSocket_postRecv \n ";
...
err_invalidateSock:
   printf("invalidating connection\n");
...
{  
   std::cout<<"go into IBVSocket_recvT \n";
   int checkRes;
...
bool _RDMASocket_connectByIP(Socket* thisone, struct in_addr ipaddress, unsigned short port)
{
   ...
   // connected
   // printf("-------------------成功完成IBVSocket_connectByIP--------------\n");
...
ssize_t _RDMASocket_recvT(Socket* thisone, iovec* iter, int flags, int timeoutMS)
{
   ...
   std::cout<<"go into _RDMASocket_recvT \n";
   ...
   //      printf("%02x ", ptr[i]); // 以十六进制格式打印每个字节
   //      if ((i + 1) % 16 == 0) {
   //          printf("\n"); // 每16个字节换行，为了可读性
   //      }
   //  }
   ...
   fhgfs_sockaddr_in *to)
{
   ...
   std::cout<<"out of IBVSocket_send \n";
...
bool RDMASocket_init(RDMASocket* thisoneone, struct in_addr src, NicAddressStats* nicStats)
{
   ...
   //    printf("empty commContext!!!! error \n");
   // }
   // printf("after RDMASocket_init %d\n", thisoneone->ibvsock.commContext->incompleteSend.numAvailable);
   return true;

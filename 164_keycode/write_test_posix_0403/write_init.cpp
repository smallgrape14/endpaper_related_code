...
void appendResultToCSV(const std::string& csv_file, const std::string& timestamp, 
                       int threadnum, size_t iosize_kb, const std::string& output_dir,
...
void worker_thread(int thread_id, int total_files, size_t iosize, const std::string& output_dir, void* d_buffer, 
    ...
    while (true) {
        ...
        // filename << output_dir << "/file_" << std::setfill('0') << std::setw(6) << file_idx << ".bin";
...
int main(int argc, char* argv[]) {
    ...
    for (int idx : check_files) {
        std::stringstream filename;
        // filename << output_dir << "/file_" << std::setfill('0') << std::setw(6) << idx << ".bin";
    ...
    double total_data_gb = (double)TOTAL_FILES * iosize / (1024.0 * 1024.0 * 1024.0);
    double total_data_mb = (double)TOTAL_FILES * iosize / (1024.0 * 1024.0);
    ...
    std::cout << "Total data written: " << total_data_gb << " GB (" << total_data_mb << " MB)" << std::endl;
    std::cout << "Total time: " << total_time_sec << " seconds" << std::endl;
    std::cout << "Total throughput: " << throughput_mbps << " MB/s" << std::endl;
    ...
    // 追加结果到CSV文件
    std::string csv_file = "Posix_Baseline_write_performance.csv";
    std::string timestamp = getCurrentTimestamp();
    
    appendResultToCSV(csv_file, timestamp, threadnum, iosize_kb, output_dir,

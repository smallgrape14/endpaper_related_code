...
void worker_thread_optimized(int thread_id, int total_files, size_t iosize, 
    ...
    // 初始化缓冲区
    std::fill_n(h_buffer, iosize, static_cast<char>(thread_id % 1024));
...
int main(int argc, char* argv[]) {
    ...
    if (threadnum <= 0 || threadnum > 1024) {
        std::cout << "Error: threadnum must be between 1 and 1024" << std::endl;
        return 1;
    }
    ...
    // 追加结果到CSV文件
    std::string csv_file = "OPT_POSIX_Baseline_write_performance.csv";

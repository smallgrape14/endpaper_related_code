...
// 建立连接，创建QP，注册内存，发布接收请求
void build_connection(struct rdma_cm_id *id)//传入对应的事件ID
{
  struct connection *conn;
  struct ibv_qp_init_attr qp_attr;

  build_context(id->verbs); // 建立上下文
  build_qp_attr(&qp_attr); // 初始化QP属性
...
void on_completion(struct ibv_wc *wc)
{
  ...
  printf("on_completion: status is %d.\n", wc->status);
  ...
  if (conn->send_state == SS_MR_SENT && conn->recv_state == RS_MR_RECV) {//write
    ...
    sge.addr = (uintptr_t)conn->rdma_local_region;//放着要写入服务端的信息
    sge.length = RDMA_BUFFER_SIZE;
    sge.lkey = conn->rdma_local_mr->lkey;

    TEST_NZ(ibv_post_send(conn->qp, &wr, &bad_wr));//发送消息，主要是传递 存放 需要写入服务端文件的信息内容

    conn->send_msg->type = MSG_DONE;
    send_message(conn);

  } else if (conn->send_state == SS_DONE_SENT && conn->recv_state == RS_DONE_RECV) {//read
...
void post_receives(struct connection *conn)
{
  ...
  // bad_wr: 指向第一个失败的工作请求的指针
  struct ibv_sge sge;

  wr.wr_id = (uintptr_t)conn;/* User defined WR ID ,用户定义的工作请求标识符*/
  wr.next = NULL;/* Pointer to next WR in list, NULL if last WR */
  wr.sg_list = &sge;/* Pointer to the scatter-gather array,该数组包含接收缓冲区的信息 */
  wr.num_sge = 1;/* Size of the scatter-gather array ,散列-聚集数组的大小*/

  sge.addr = (uintptr_t)conn->recv_msg;/* 数据段所在的虚拟内存的起始地址 */
  sge.length = sizeof(struct message);/* 数据段长度 */
  sge.lkey = conn->recv_mr->lkey; /* 该数据段对应的本地内存区域键 (L_Key) */
